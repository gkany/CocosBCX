...
def proposal(account, expire_time=None, changed_param=None):
    if not expire_time:
        ...
        #  将时间戳转换为字符串
        expire_time = time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime(maint_time - 90))
    ...
    result = propose_extension_parameter(account, expire_time, changed_param)
    tx_id = result[0]
...
def approve_proposal(propose_id, account, approve=None):
    try:
        if not approve:
